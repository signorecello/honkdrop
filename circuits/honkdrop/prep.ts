...
export const prepareInputs = async () => {
  ...
  const inputs = {
    ...
    c: Array.from(hexToUint8Array(plume.c)),
    s: Array.from(hexToUint8Array(plume.s)),
    pk_x: Array.from(pk.slice(0, 32)),
    pk_y: Array.from(pk.slice(32, 64)),
    nullifier_x: Array.from(hexToUint8Array(nullifier.toHex())).slice(0, 32),
    nullifier_y: Array.from(hexToUint8Array(nullifier.toHex())).slice(32, 64),

    merkle_path: mtProof.siblings,
    index: mtProof.index,
    merkle_root: mtProof.root,
    claimer_priv: '0x8aEC1f81CB90204e3C9Aa1Aeed2915C181CbBe28',
    claimer_pub: '0x8aEC1f81CB90204e3C9Aa1Aeed2915C181CbBe28',
  };
